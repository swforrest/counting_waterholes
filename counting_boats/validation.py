...
def main(classifications, labels, outdir):
    ...
    if os.path.exists(os.path.join(folder, "all_boats.csv")):
        os.remove(os.path.join(folder, "all_boats.csv"))
    with open(os.path.join(folder, "all_boats.csv"), "w+") as file:
        file.write("lat, long, ml_class, manual_class, filename\n")
    ...
    # summarize the aois
    csvs = [os.path.join(summaries, file) for file in os.listdir(summaries) if file.endswith(".csv") and "summary" not in file] 
    aois = list(set([file.split("_")[1].split(".")[0] for file in csvs]))
    for aoi in aois:
        summary_aoi(summaries, aoi)
...
def summary_aoi(outdir, aoi):
    """
    Open all csv files relating to the given aoi and summarize them
    """
    csvs = [os.path.join(outdir, file) for file in os.listdir(outdir) if file.endswith(".csv") and aoi in file and "summary" not in file]
    # we want: sample size (labels), sample size (detected), true positives, false positives, false negatives
    # get all lines (except headers) from all files using np list comp
    all_boats = np.asarray([line.strip().split(",") for file in csvs for line in open(file) if line[0] != "x"])
    if len(all_boats) == 0:
        print("No boats found")
        return
    pred = all_boats[:, 2].astype(float).astype(int)
    true = all_boats[:, 3].astype(float).astype(int)
    total_labelled = len(true[true != -1])
    # get the confusion matrix
    f1 = f1_score(y_pred=pred, y_true=true, average="weighted")
    acc = np.sum(true == pred) / len(true)
    ConfusionMatrixDisplay.from_predictions(y_pred=pred, y_true=true,
            labels=[-1, 0, 1], display_labels=["Undetected", "Static Boat", "Moving Boat"])
    fig = plt.gcf()
    aoi = aoi[0].upper() + aoi[1:]
    fig.suptitle(f"{aoi}: {total_labelled} Labelled Boats (Accuracy: {round(acc, 3)})")
    fig.tight_layout()
    # save the confusion matrix image
    plt.savefig(os.path.join(outdir, f"confusion_matrix_{aoi}.png"))
    # save a summary csv
    with open(os.path.join(outdir, f"summary_{aoi}.csv"), "w+") as file:
        file.write("class, total_boats_labelled, total_boats_detected, real_boats,  \
                detected_boats, im_per_boat_detected, im_per_boat_labelled,         \
                true_positives, false_positives, false_negatives\n")
        # all
        file.write(f"all, {len(true[true != -1])}, {len(pred[pred != -1])}, {len(true[true != -1])},    \
                {len(pred[pred != -1])}, {len(pred[pred != -1])/len(true[true != -1])},                 \
                {len(true[true != -1])/len(true[true != -1])}, {np.sum(true == pred)},                  \
                {len(pred[(pred != -1) & (true == -1)])},                                               \
                {len(pred[(pred == -1) & (true != -1)])}\n")
        # static
        file.write(f"static, {len(true[true == 0])}, {len(pred[pred == 0])}, {len(true[true == 0])},  \
                {len(pred[pred == 0])}, {len(pred[pred == 0])/len(pred[pred == 0])},             \
                {len(true[true == 0])/len(true[true == 0])}, {np.sum((true == pred) & (true == 0))}, \
                {len(pred[(pred == 0) & (true != 0)])},                                             \
                {len(true[(pred != 0) & (true == 0)])}\n")
        # moving
        file.write(f"moving, {len(true[true == 1])}, {len(pred[pred==1])}, {len(true[true == 1])},  \
                {len(pred[pred==1])}, {len(pred[pred==1])/len(pred[pred==1])},                     \
                {len(true[true == 1])/len(true[true == 1])}, {np.sum((true == pred) & (true == 1))}, \
                {len(pred[(pred == 1) & (true != 1)])},                                             \
                {len(true[(pred != 1) & (true == 1)])}\n")
...
def write_to_csv(boats, filename, outdir):
    ...
    # also write the lat long to an overall csv
    to_latlong(boats, filename)

def to_latlong(boats, filename):
    """
    Convert x and y of an image to latlong
    """
    # find the image
    image = None
    for root, dirs, files in os.walk(os.path.join(folder, "RawImages")):
    ...
    for boat in boats:
        ...
        boat[0] = lat
        boat[1] = long
    # append the baots to the 'all_boats.csv'
    with open(os.path.join(folder, "all_boats.csv"), "a") as file:
        for boat in boats:
            file.write(f"{boat[0]}, {boat[1]}, {boat[2]}, {boat[3]}, {filename}\n")

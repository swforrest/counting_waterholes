...
def add_udm_clear_to_tif(udm_path: str, tif_file):
    ...
    # New tif with the full bounds of both
    max_width = max(udm.RasterXSize, tif.RasterXSize)
    max_height = max(udm.RasterYSize, tif.RasterYSize)
    # Create the new raster
    driver = gdal.GetDriverByName("GTiff")
    out_raster = driver.Create("clear.tif", max_width, max_height, 1, gdal.GDT_Byte)
    # Set the projection and geotransform
    out_raster.SetProjection(tif.GetProjection())
    out_raster.SetGeoTransform(tif.GetGeoTransform())
    # Read the rasters
    in_band_udm = udm.GetRasterBand(1)
    in_band_tif = tif.GetRasterBand(1)
    out_band = out_raster.GetRasterBand(1)
    ...
    # Need them to be the same size
    if udm_array.shape != tif_array.shape:
        # resize both to the max size, position the smaller one in the correct location

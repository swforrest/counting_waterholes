...
def prepare(
    ...
    tif_files = [
        os.path.join(root,f) for root, _, files in os.walk(cfg["raw_images"]) for f in files if f.endswith(".tif")
    ]
    ...
    for i, tif in enumerate(tif_files):
        ...
        ics.create_padded_png(tif_dir, cfg["output_dir"], tif_name, cfg["TILE_SIZE"], cfg["STRIDE"])
...
def segment(
    config: str = typer.Option("", help="Path to the config file"),
    train_val_split: float = typer.Option(0.8, help="Proportion of images to use for training")
):
    """
    Segment the images
    """
    ...
    labels = [os.path.join(cfg["output_dir"], f) for f in os.listdir(cfg["output_dir"]) if f.endswith(".json")]
    ...
    for i, (image, label) in enumerate(zip(images, labels)):
        ics.segment_image(image, label, cfg["TILE_SIZE"], cfg["STRIDE"], im_outdir=image_out, labels_outdir=label_out)
...
def train(
    ...
    os.system(f"{cfg['python']} {cfg['yolo_dir']}/train.py \
              --img {cfg['TILE_SIZE']} --batch {cfg['BATCH_SIZE']} \
              --workers {cfg['workers']} \
              --epochs {cfg['EPOCHS']} --data {cfg['data']} \
              --weights {cfg['weights']} --cache {cfg['cache']}")
